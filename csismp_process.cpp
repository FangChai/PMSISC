...
static bool check_valid(session to_check);
static void build_student_session(session & to_build,session_type type,size_t size);
static void print_all_students();
static session construct_ackmsg(session * _session);
static session construct_rjtmsg(session * _session);
